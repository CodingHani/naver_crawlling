...
idx_drop=df[df['CountDong'] == 1].index # 동 갯수가 1개인 아파트(나홀로 아파트)는 제외하기

df_ApartNumber=df.drop(idx_drop)
print(len(df_ApartNumber))
pass_or_not=input("최대 예상 소요 시간은 " + str(len(df_ApartNumber)*8) + "초 입니다. 엔터를 누르면 진행합니다.")
...
for i in (dic_ApartNumber['MarkerId']) : # 20개씩 밖에 검색이 안되기 때문에 아파트별 물건수에 따른 페이지수 부터 검색 # 
...
Last_drop=df_Selling[(df_Selling['ExclusiveSize']<84)&(df_Selling['ExclusiveSize']>86)].index # 전용 기준 필요 없는 조건 제외
...
n=0
pd.set_option('mode.chained_assignment',  None) # <==== 경고 셧 다운 (밑에서 새로운 DataFrame을 너무 많이 만들어서 경고성 메시지가 뜨는듯...)
